        ...
        <div class="bg-white p-8 shadow-md rounded-lg w-full max-w-md">
            ...
            <form action="{{ route('admin.transactions.update', $transaction->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount" id="amount" step="0.01" min="0.01" max="99999999.99" value="{{ $transaction->amount }}" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                    @error('amount')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
